...
AS FItemSecCoefficient,g.FDate,h.FDate 
...
left join (select b.FItemID,max(a.FDate) as FDate from ICStockBill a left join ICStockBillEntry b on a.FInterID=b.FInterID where FTranType=21 group by b.FItemID) g on g.FItemID=u1.FItemID
left join (select b.FItemID,max(a.FDate) as FDate from ICStockBill a left join ICStockBillEntry b on a.FInterID=b.FInterID where FTranType=2 group by b.FItemID) h on h.FItemID=u1.FItemID
where (Round(u1.FQty,t1.FQtyDecimal)<>0 OR 
Round(u1.FQty/t4.FCoefficient,t1.FQtyDecimal)<>0) 
 and t1.FDeleted=0 
--AND t2.FTypeID in (/*500,20291,20293,*/9526)
...
--and g.FDate<='2010-05-01'
and g.FDate is null and h.FDate is null
--and t1.FNumber = '06.04.4003'
and t1.FErpClsID=2
