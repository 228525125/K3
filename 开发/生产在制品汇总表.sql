--drop procedure report_zzphz

create procedure report_zzphz
@query nvarchar(255)
as 
begin
SET NOCOUNT ON 
...
if @query=''
select wldm,wlmc,wlgg,dw,sum(llsl)+sum(blsl)-sum(bfsl) as 'llsl',sum(zzpsl) as 'zzpsl' from #Data group by wldm,wlmc,wlgg,dw
else
select wldm,wlmc,wlgg,dw,sum(llsl)+sum(blsl)-sum(bfsl) as 'llsl',sum(zzpsl) as 'zzpsl' from #Data where wldm like '%'+@query+'%' group by wldm,wlmc,wlgg,dw
end


execute report_zzphz ''
