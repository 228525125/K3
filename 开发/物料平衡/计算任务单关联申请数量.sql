--计算任务单已申请数量（FCheckCommitQty、FAuxCheckCommitQty）,已申请数量 = 计划 - (已领 - 报废 + 退料（2013-06-26之前） - 申请)，安装触发器：生产报废单审核、反审核；生产领料退料审核、反审核；检验申请保存、删除；生产任务单下达；
...
SELECT a.FBillNo,
max(a.FQty - (ROUND(case when (b.FAuxStockQty + ISNULL(-f.FQty,0) + ISNULL(-k.FQty,0))>b.FAuxQtyMust then b.FAuxQtyMust else (b.FAuxStockQty + ISNULL(-f.FQty,0) + ISNULL(-k.FQty,0)) end/b.FAuxQtyScrap,0) - ROUND(b.FDiscardAuxQty/b.FAuxQtyScrap,0) + ISNULL(-e.FQty,0) - ISNULL(c.FQty,0))) as '计算结果'
FROM ICMO a
...
and a.FQty -(ROUND(case when (b.FAuxStockQty + ISNULL(-f.FQty,0) + ISNULL(-k.FQty,0))>b.FAuxQtyMust then b.FAuxQtyMust else (b.FAuxStockQty + ISNULL(-f.FQty,0) + ISNULL(-k.FQty,0)) end/b.FAuxQtyScrap,0) - ROUND(b.FDiscardAuxQty/b.FAuxQtyScrap,0) + ROUND(ISNULL(-e.FQty,0)/b.FAuxQtyScrap,0) - ISNULL(c.FQty,0)) <> a.FCheckCommitQty   --计划 - (已领 + 退料（2015-02-01之后） - 报废 + 退料（2013-06-26之前） - 申请)
