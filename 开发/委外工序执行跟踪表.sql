--drop procedure report_wwgxzxgzb drop procedure report_wwgxzxgzb_count

create procedure report_wwgxzxgzb
@query varchar(50),
...
Where 1=1  
union 
select * from #Data where FSumSort=101
order by FSumSort,FDate
end

--count--
create procedure report_wwgxzxgzb_count
@query varchar(50),
@begindate varchar(10),
@enddate varchar(10)
as 
...
,FEntryID int default(0)--转出单分录内码
,FSumSort int default(0)
,FNotPassQty decimal(28,10) default(0)
,FQINQty decimal(28,10) default(0)
,FQOUTQty decimal(28,10) default(0)
)
...
select convert(datetime,convert(varchar(30),t1.FBillDate,111)) as FDate,t1.FBillNo as FOutBill, case when t2.FCloseFlag=1 then 'Y' else 'N' end ,t3.FNumber as 
FInvoiceNumber,t3.FName as FInvoiceName
,t4.FName as FEmpName, t5.FNumber as FNumber , t5.FShortNumber as FShortNumber,t5.FName,t5.FModel as FModel
,t6.FID as FOper,t6.FName as FOperName ,convert(datetime,convert(varchar(30),t2.FFetchDate,111)) as FDeliDate, t7.FName as FUnit
,t2.FTranOutQty as FOutQty ,t2.FOperTranOutQty as FOperOutQty,t2.FReceiptQty as FInQty,t2.FQualifiedQty as FQualifiedQty,t2.FEntryID,isnull(t20.FNotPassQty,0),t21.FQty as FQINQty,t22.FQty as FQOUTQty 
from ICShop_SubcOut t1
inner join ICShop_SubcOutEntry t2 on t1.FInterID=t2.FInterID
left join t_Supplier t3 on t1.FSupplierID=t3.FItemID
left join t_Emp t4 on t1.FEmpID=t4.FItemID
...
left join t_Measureunit t7 on t2.FUnitID=t7.FMeasureUnitID
left join (
select FNotPassQty,FSHSubcOutInterID,FSHSubcOutEntryID from ICQCBill
where 1=1
AND FTranType=715 AND FCancellation = 0 AND FStatus=1
) t20 on t20.FSHSubcOutInterID=t2.FInterID and t20.FSHSubcOutEntryID=t2.FEntryID
left join (
select b.FNote,sum(b.FQty) as 'FQty' from ICStockBill a left join ICStockBillEntry b on a.FInterID=b.FInterID where a.FTranType=10 and a.FCheckerID>0 and a.FCancellation=0 and b.FNote like 'WWZC%' group by b.FNote
) t21 on t21.FNote=t1.FBillNo
...
execute report_wwgxzxgzb '','2011-06-01','2011-09-30'

execute report_wwgxzxgzb_count '','2011-09-01','2011-09-30'


select t2.FReceiptQty from ICShop_SubcOut t1
inner join ICShop_SubcOutEntry t2 on t1.FInterID=t2.FInterID
where 1=1

select FNotPassQty,FSHSubcOutInterID,FSHSubcOutEntryID,* from ICQCBill
where 1=1
AND FBillNo in ('SIPQC001091','SIPQC001099')
AND FTranType=715 AND FCancellation = 0 AND FStatus=1 AND FResult<>13556

select * from ICShop_SubcOut a left join ICShop_SubcOutEntry b on a.FInterID=b.FInterID
left join ICQCBill c on c.FSHSubcOutInterID=b.FInterID and c.FSHSubcOutEntryID=b.FEntryID
where 1=1
and a.FBillNo like '%WWZC000912%'


select b.FNote from ICStockBill a left join ICStockBillEntry b on a.FInterID=b.FInterID where a.FCheckerID>0 and a.FCancellation=0 and b.FNote like 'WWZC%'

select * from ICStockBill a left join ICStockBillEntry b on a.FInterID=b.FInterID where a.FBillNo like 'QOUT%' and a.FCheckerID>0 and a.FCancellation=0