...
AS FItemSecCoefficient,iii.FEntrySelfA0160
...
left join (select i.FNumber,u1.FBatchNo,u1.FEntrySelfA0160 from ICStockBill v1 INNER JOIN ICStockBillEntry u1 ON     v1.FInterID = u1.FInterID   AND u1.FInterID <>0 LEFT JOIN t_ICItem i on u1.FItemID=i.FItemID where 1=1 AND v1.FTranType=1 AND  v1.FCancellation = 0 group by i.FNumber,u1.FBatchNo,u1.FEntrySelfA0160) iii on t1.FNumber=iii.FNumber AND iii.FBatchNo=u1.FBatchNo
 where (Round(u1.FQty,t1.FQtyDecimal)<>0 OR 
Round(u1.FQty/t4.FCoefficient,t1.FQtyDecimal)<>0) 
 and t1.FDeleted=0 

 AND left(t1.FNumber,5)='01.01' 
